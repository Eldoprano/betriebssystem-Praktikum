...
func HalStart(instr map[int]string, d bool, buffer chan string, wg *sync.WaitGroup, inType int, outType int) {
	defer wg.Done()
	if d {
		debug = true
	...
	for i := 1; i <= len(instr)-1; i++ {
		...
		case "STOP":
			stop()
		case "OUT":
			out(parameter, outType, buffer)
		case "IN":
			infunc(parameter, inType, buffer)
		case "LOAD":
			load(parameter)
		case "LOADNUM":
			loadnum(parameter)
...
func infunc(parameter string, inputType int, buffer chan string) {
	...
	fmt.Println("trying to receive inputType:", inputType)
	...
	fmt.Println("done Receiving input")
}
...
func out(parameter string, outputType int, buffer chan string) {
	number, err := strconv.Atoi(parameter)
	if err != nil {
