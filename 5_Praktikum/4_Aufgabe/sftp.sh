...
get_files=$(sftp $USERNAME@$IP_ADDRESS:$DIRECTORY"*" ".")

# Try to make a connection 3 times. If it fails the 3 times, it exits with error and logs it to sftp.log
get_files || get_files || get_files || echo "Keine Verbindung mit den Server" >> sftp.log; echo "Failed. view sftp.log for details"; exit 1;
